...
API_HASH = getenv("API_HASH", "dfcgvbhtrfdgujjhvfybfghbgtghbfgy")
...
Owner = "Shikari"
repo = "https://github.com/ShikariBaaZ/Ban_Bot"
@sree.on(events.NewMessage(pattern="^/start"))
async def start(event):
    buttns = [Button.url("••ѕυρροяτ••", "https://t.me/ShikariSupportNetwork"), Button.url("••ʀєρο••", f'{repo}')]
    py = platform.python_version()
    if event.sender.id in OP:
        await sree.send_file(
            event.chat.id,
            file="https://telegra.ph/file/bc754951ddc1d771f4768.jpg",
...
async def start(event):
    ...
    if event.sender.id not in OP:
        await event.reply(
            "Huh Nigga!\nThis is not for you lol 😑\n\nMake your own bot from this [Repository⚡](https://github.com/ShikariBaaZ/Ban_Bot)",
            link_preview=False,
        )       
  ...
  if event.sender.id in OP:
   if not event.is_group:
       ...
       if not admin and not creator:
           ...
           return
       hmm =  await event.reply("__Yeah! Black Magic Begins🥳...__")
       await sleep(18)
       await hmm.delete()
...
print("Join @ShikariSupportNetwork if you facing any kind of issue!!")
